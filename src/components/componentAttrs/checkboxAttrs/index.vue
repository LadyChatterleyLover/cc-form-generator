<template>
  <el-form label-width="90px">
    ...
    <el-form-item label="复选框样式">
      <el-select v-model="buttonType" placeholder="请选择复选框样式" @change="changeButtonType">
        ...
        <el-option value="checkbox-button" label="按钮"></el-option>
      </el-select>
    </el-form-item>
    <template v-if="buttonType === 'checkbox-button'">
  ...
  <el-dialog v-model="editVisible" title="编辑数据源" width="30%">
    <el-form v-if="current" :model="current.children[currentIndex]" label-width="100px">
      ...
      <el-form-item label="选项尺寸" v-if="current.children[currentIndex].type === 'checkbox-button'">
      ...
      <el-form-item label="显示边框" v-if="current.children[currentIndex].type === 'checkbox'">
